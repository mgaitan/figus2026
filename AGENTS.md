...
Repository: https://github.com/mgaitan/figus2026

Description: Cloud sticker album for the 2026 World Cup.

## Product

- The app is a free, cloud-hosted World Cup 2026 sticker album.
- The first screen should be the album experience, not a marketing page.
- Collectors browse country by country, see owned and missing stickers, and open daily packs.
- Stickers should evoke a physical football sticker album: consistent borders, player photo area, country/team identity, and rarity.
- The MVP uses deterministic seed data for Argentina, Brazil, and France while the Wikipedia scraper matures.
- Keep product decisions in GitHub issues. Use the `mvp` label for the smallest playable scope.

## Stack

- **Python:** Python 3.14 target, currently compatible with the versions declared in CI. Check main dependencies in `pyproject.toml`.
- **Backend:** FastAPI, SQLModel, SQLite for the MVP.
- **Admin:** fastapi-admin or the simplest equivalent admin surface for tuning countries, players, scarcity, and quiz questions.
- **Frontend:** Vite and Tailwind CSS.
- **Auth:** Prefer the lowest-friction path. Auth0 tenant `figus2026` is available and should be evaluated first for Google/Facebook social login.
- **Images:** Start from Wikipedia/Wikimedia source metadata. Avoid assuming production hotlinking is acceptable; evaluate Cloudflare R2 mirroring with Wrangler.
- **Deploy:** FastAPI Cloud is the intended production target.
...
- Documentation previews for PRs with docs changes are published under `https://mgaitan.github.io/figus2026/_preview/pr-<PR_NUMBER>/`.
- Release/manual docs publish uses the canonical site URL: `https://mgaitan.github.io/figus2026/`.
