...
from figus2026 import get_version, main
...
def test_show_help(capsys: pytest.CaptureFixture):
    ...
    assert "figus2026" in captured.out
...
def test_show_version(mocker, capsys: pytest.CaptureFixture):
    ...
    Parameters:
        ...
        capsys: Pytest fixture to capture output.
    """
    mocker.patch("figus2026.get_version", return_value="0.1.0")
    with pytest.raises(SystemExit):
        main(["-V"])
    captured = capsys.readouterr()
...
def test_main_module(mocker):
    ...
    module_name = "figus2026.__main__"
    # Simulate: python -m figus2026 --version
    mocker.patch.object(sys, "argv", ["figus2026", "-V"])
    with pytest.raises(SystemExit):
